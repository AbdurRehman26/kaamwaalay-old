			...
			<div class="job-post-details">
				...
				<div class="job-common-description job-perform">
					...
					<div class="col-md-6 job-bid-btn p-r-0">
						<a href="javascript:void(0);" @click="$emit('chatmessage', record)" class="chat-message" :class="{disable: record.job.can_message? false : true}"><i class="icon-message"></i></a>						
						<a href="javascript:void(0);" @click="$emit('chatmessage', record)" class="chat-message" :class="{disable: record.job.can_message? true : false}"><i class="icon-message"></i></a>						
...
<script>
	...
	export default {
		...
		watch: {
			show(val) {
				if(val) {
					this.url = 'api/job-bid?pagination=true&filter_by_job_detail=true&filter_by_invitation=1&filter_by_archived=0&is_status=invited';
				}else {
					//this.url = null;
