        <template>
          <div>
           <div class="panel-inner">
            <div class="row">
                <div class=" col-xs-12 col-md-12">
                    <div class="row">
                      <div class="col-xs-12 col-md-12 ">
                          <a href="javascript:;" class="btn btn-primary float-right margin-bottom-30px" @click="ShowModalUser">Add Admin</a>
                      </div>
                  </div>
              </div>
          </div>
          <div class="row">
            <div class="col-md-12">
               <div class="table-area">
                <div class="table-responsive">
                    <table class="table last-col-fix">
                      <thead>
                        <tr>
                          <th>First Name</th>
                          <th>Last Name</th>
                          <th>Email Address</th>
                          <th>Join Date</th>
                          <th>Access Level</th>
                          <th class="text-center">Status</th>
                      </tr>
                  </thead>
                  <tbody>
                    <tr v-for="list in listing">
                      <td>{{list.fname}}</td>
                      <td>{{list.lname}}</td>
                      <td><a href="javascript:;">{{list.email}}</a></td>
                      <td>{{list.jdate}}</td>
                      <td class="statustext"><a class="statustext active" @click="ChangeAccess">{{list.acesslevel}}</a></td>
                      <td class="text-center statustext">
                        <div class=""><a class="statustext active" @click="StatusChange">Active</a></div>
                    </td>
                </tr>
            </tbody>
        </table>
    </div>
</div>
</div>
<div class="col-xs-12 col-md-12">
    <div class="total-record float-left">
        <p><strong>Total records: <span>3</span></strong></p>
    </div>
</div>
</div>

<add-new-user @HideModalValue="HideModal" :showModalProp="showModalValue"></add-new-user>
<confirmation @HideModalValue="HideModal" :showModalProp="changestatus"></confirmation>
<confirmation-access @HideModalValue="HideModal" :showModalProp="changeAccess"></confirmation-access>
</div>
</div>
</template>
<script>
    export default {
      data () {
        return {
           showModalValue: false,
           changestatus: false,
           changeAccess: false,
           actiondelete: false,
           pageTitle:'Admin',

           listing: [
           {
            fname:'Dickerson',
            lname:'Macdonald',
            email:'dmacdonald@gmail.com',
            acesslevel:'Full',
            jdate: '22-01-2018',
            status: 'Active',
        },
        {
            fname:'Larsen',
            lname:'Shaw',
            email:'shawlarsen@gmail.com',
            acesslevel:'Review',
            jdate: 'July 1, 2018',
            status: 'Active',
        },
        {
            fname:'Geneva',
            lname:'Wilson',
            email:'genevawilson@gmail.com',
            acesslevel:'Full',
            jdate: 'July 2, 2018',
            status: 'Deactive',
        },
        ],

    }
},
methods: {
    ShowModalUser(){
        this.showModalValue = true;
    },
    StatusChange(){
        this.changestatus = true;
    },
    ChangeAccess(){
        this.changeAccess = true;
    },
    HideModal(){
        this.showModalValue = false;
        this.changestatus = false;
        this.changeAccess = false;
    },
    statusLink(event) {
        if(event.target.className == "active")
        {
            event.target.className = "deactive";
            event.target.text = "Deactive";
        }
        else
        {
            event.target.className = "active";
            event.target.text = "Active";
        }
    }
}
}
</script>
